...
// ---------- DO NOT CHANGE ANYTHING IN THIS FILE ABOVE THIS LINE ----------
  ...
  static Node *copy_nodes_impl(Node *node) {
    ...
    return copy;
      ...
      return nullptr;  //TEST THIS FUNCTION LATER!!!!!!!!!!!!!!!
    }
  }
  ...
  static Node * max_element_impl(Node *node) {
    ...
    else if(node != nullptr && node->right == nullptr){
         return node; 
    }
    else{
      return nullptr;
  ...
  static Node * min_greater_than_impl(Node *node, const T &val, Compare less) {
    if (node != nullptr){
      if(!node->left){
        ...
        less(val, node->left->right->datum)){
          return min_greater_than_impl(node->left,val, less);
        }
        ...
        else if(node->left->right && less(val, node->left->right->datum)){
          return min_greater_than_impl(node->left, val, less);
        }
        else{
          return node;

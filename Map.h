...
private:
  ...
  class PairComp {
     public:
     bool operator()(const Pair_type &fir, const Pair_type &sec){
      Key_compare comp = Key_compare();
      return comp(fir.first, sec.first);
     }
  };
...
public:
  ...
  Iterator find(const Key_type& k) const{
     Pair_type dummy = Pair_type(k, Value_type());
  ...
  Value_type& operator[](const Key_type& k){
     if(find(k) != end()){
        ...
        Pair_type insert = Pair_type(k, Value_type());
        tree.insert(insert);
        return (*tree.find(insert)).second;
     }
  }
  ...
  std::pair<Iterator, bool> insert(const Pair_type &val){
    if(find(val.first) != end()){
      std::pair<Iterator, bool> i = {find(val.first), false};
      return i;
    }
    else{
      tree.insert(val);
      std::pair<Iterator, bool> i = {find(val.first), true};
      return i;
    }
  }
